    ...
    <Shell>
      <header className="border-b border-[#e7ece3]">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-moss">Hadiri by TAPTU</p>
      ...
      <main>
        <section className="relative overflow-hidden">
          <div className="absolute inset-x-0 top-0 h-[520px] bg-[radial-gradient(circle_at_top,_rgba(45,154,118,0.14),_transparent_55%)]" />
          <div className="mx-auto grid max-w-7xl gap-12 px-6 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:px-10 lg:py-20">
            <div className="relative z-10">
              <p className="inline-flex items-center rounded-full border border-[#d7e6dc] bg-white/80 px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-moss shadow-sm backdrop-blur">
                PWA mobile-first + login website
              </p>
              <h1 className="mt-7 max-w-2xl text-4xl font-semibold leading-[1.08] tracking-[-0.03em] text-ink md:text-5xl lg:text-6xl">
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                {metrics.map((metric) => (
                  <div key={metric.label} className="rounded-[28px] border border-[#e2e8de] bg-white/80 p-5 shadow-panel">
                    <p className="text-2xl font-semibold tracking-[-0.04em] text-ink">{metric.value}</p>
              ...
              <div className="rounded-[36px] border border-[#dae5db] bg-[#fdfefc] p-4 shadow-panel md:p-6">
                <div className="rounded-[28px] bg-ink p-6 text-white">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs uppercase tracking-[0.24em] text-[#9fd7c0]">Admin workspace</p>
                      <h2 className="mt-3 text-2xl font-semibold">Monitor kehadiran tanpa tampilan yang berantakan.</h2>
                    </div>
                    <div className="rounded-full bg-white/10 px-4 py-2 text-sm">Live sync</div>
                  </div>
                  ...
                  <div className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
                    ...
                    <div className="rounded-[26px] bg-[#132822] p-5">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-semibold text-white">Tren kehadiran</p>
                        ...
                        {[52, 64, 73, 68, 81, 85, 92].map((value, index) => (
                          <div key={index} className="flex flex-1 flex-col items-center gap-3">
                            <div className="w-full rounded-full bg-white/8 p-1">
                              <div className="rounded-full bg-gradient-to-t from-[#1fa178] to-[#9af0ce]" style={{ height: `${value}%` }} />
                            </div>
          ...
          <div className="grid gap-5 lg:grid-cols-3">
            {features.map((feature) => (
              <article key={feature.title} className="rounded-[28px] border border-[#dde6dd] bg-white p-7 shadow-panel">
                ...
                <h3 className="mt-6 text-2xl font-semibold tracking-[-0.03em] text-ink">{feature.title}</h3>
